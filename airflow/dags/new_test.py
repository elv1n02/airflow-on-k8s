...
def mysql_to_parquet_and_store_in_minio():
    ...
    # Time the MySQL read operation
    start_time = time.time()
    cursor.execute(query)
    data = cursor.fetchall()
    columns = [col[0] for col in cursor.description]  # Get column names
    df = pd.DataFrame(data, columns=columns)
    mysql_read_time = time.time() - start_time  # Calculate MySQL read time
    
    # Time the Parquet conversion operation
    start_time = time.time()
    table = pa.Table.from_pandas(df)
    parquet_conversion_time = time.time() - start_time  # Calculate Parquet conversion time
    ...
    object_name = 'users_new.parquet'  # Path in the MinIO bucket
    
    # Time the writing to MinIO operation
    start_time = time.time()
    s3_hook.load_file_obj(parquet_buffer, object_name, bucket_name, replace=True)
    minio_write_time = time.time() - start_time  # Calculate writing time to MinIO
    
    # Print the timing results
    print(f"MySQL read time: {mysql_read_time:.4f} seconds")
    print(f"Parquet conversion time: {parquet_conversion_time:.4f} seconds")
    print(f"Writing to MinIO time: {minio_write_time:.4f} seconds")
    print(f"Parquet file successfully uploaded to {bucket_name}/{object_name}")

# Define the DAG
dag = DAG(
    'select_user_with_time',
